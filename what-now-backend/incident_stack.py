...
GAP_TO_FIELD: dict[str, str] = {
    "injuries": "injuries",
    "still_at_scene": "still_at_scene",
...
# Used when both question variants were asked but the gap is still open.
GAP_DEFAULTS: dict[str, Any] = {
    "injuries": "minor",
    "still_at_scene": True,
    "incident_type": "other",
    "state": "unknown",
...
class IncidentStack:
    ...
    def update_from_transcript(self, transcript: str) -> None:
        if transcript.strip():
            self.data["turns"] += 1
        self._extract_from_text(transcript)

    def absorb_user_text(self, transcript: str) -> None:
        """Parse user text without incrementing turn count (for Vapi history replay)."""
        self._extract_from_text(transcript)

    def _apply_short_answer_heuristics(self, transcript: str) -> None:
        """Map yes/no and short replies to the current open gap only."""
        ...
        yes = t in {
            ...
            "somewhat",
            "maybe",
        } or t.startswith("yes ") or t.startswith("yeah ")
        no = t in {
            "no",
            "nope",
            "nah",
            ...
            "dont",
            "no i",
        } or t.startswith("no ") or "not hurt" in t or "i'm okay" in t or "im okay" in t

        if gap == "injuries":
            if no:
                self.data["injuries"] = "none"
            elif yes or any(w in t for w in ("hurt", "pain", "injured", "sore", "ache", "bleeding")):
                self.data["injuries"] = "moderate"
        elif gap == "still_at_scene":
            if yes or any(w in t for w in ("still here", "still at", "at the scene", "i'm here")):
                self.data["still_at_scene"] = True
            elif no or any(w in t for w in ("left", "went home", "i'm home", "im home")):
                self.data["still_at_scene"] = False
        elif gap == "signed_anything":
            if yes or "signed" in t:
                self.data["signed_anything"] = True
            elif no or "didn't sign" in t or "did not sign" in t or "haven't signed" in t:
                self.data["signed_anything"] = False
        elif gap == "witnesses":
            if yes or "witness" in t or "someone saw" in t:
                self.data["witnesses"] = True
            elif no or "no one saw" in t or "nobody saw" in t:
                self.data["witnesses"] = False

    def _extract_from_text(self, transcript: str) -> None:
        t = transcript.lower()
        self._apply_short_answer_heuristics(transcript)
    ...
    def to_agent_context(self) -> dict[str, Any]:
        ...
        ctx: dict[str, Any] = {
            ...
            "nearby_medical_fired": bool(self.data.get("nearby_medical_fired")),
    ...
    def sync_from_agent_context(self, context: dict[str, Any]) -> None:
        ...
        if context.get("nearby_medical_fired"):
            self.data["nearby_medical_fired"] = True
...
def get_next_question(stack: IncidentStack) -> str | None:
    while True:
        ...
        # Both variants asked but gap still open — apply default and move on.
        field = GAP_TO_FIELD.get(top_gap)
        default = GAP_DEFAULTS.get(top_gap)
        if field and stack.data.get(field) is None and default is not None:
            stack.data[field] = default
            continue
