...
_VERBOSE_ACK_RE = re.compile(
    r"sorry to hear|i understand|it's good|it\u2019s good|that sounds|i'm sorry|im sorry|"
    r"this is scary|completely normal|overwhelming|good that you|glad you",
    re.I,
)


def _crisp_ack(transcript: str) -> str:
    """One short warm line — never a paragraph."""
    t = (transcript or "").strip().lower()
    if not t or t == "__start__":
        return "I'm here."
    if t in ("yes", "yeah", "yep", "yup", "no", "nope", "nah", "ok", "okay", "sure"):
        return "Got it."
    if any(w in t for w in ("scared", "panic", "help", "please", "crying", "shaking", "confused")):
        return "I'm here."
    if any(w in t for w in ("hurt", "pain", "bleeding", "neck", "back", "head", "wrist", "broken", "ache")):
        return "Okay."
    if any(w in t for w in ("rear", "hit", "crash", "accident", "fell", "slip", "collid", "t-bone")):
        return "I hear you."
...
def _build_crisp_questioning_response(transcript: str, next_q: str) -> str:
    q = next_q.strip()
    if not q.endswith("?"):
        q = f"{q}?"
    return f"{_crisp_ack(transcript)} {q}"


def _finalize_questioning_response(content: str, next_q: str, transcript: str = "") -> str:
    """Force questioning to short ack + exact next question only."""
    q = next_q.strip()
    if not q.endswith("?"):
        q = f"{q}?"
    sentences = [p.strip() for p in re.split(r"(?<=[.!?])\s+", (content or "").strip()) if p.strip()]
    ack = sentences[0] if sentences else _crisp_ack(transcript)
    advice_markers = (
    ...
    if (
        not ack
        or len(ack.split()) > 6
        or _VERBOSE_ACK_RE.search(ack)
        or any(m in ack.lower() for m in advice_markers)
    ):
        ack = _crisp_ack(transcript)
    return f"{ack} {q}".strip()


def _enforce_questioning_no_advice(content: str, next_q: str, transcript: str = "") -> str:
    """Strip advice sentences from questioning responses; keep ack + question."""
    return _finalize_questioning_response(content, next_q, transcript)


def _enforce_questioning_brevity(content: str, next_q: str = "", transcript: str = "") -> str:
...
[One SHORT warm line — 2-4 words max, e.g. "Got it." "I hear you." "Okay."]
[The exact question you were given — word for word]

Example:
"Got it. Are you still at the scene right now?"

NOT:
"I'm sorry to hear about the accident and your neck pain. Are you still at the scene?"
NOT:
...
The user has urgent care facilities visible on their screen right now.
Include this naturally in your response:

"Even if you feel okay right now — get checked today, not tomorrow. I've pulled up the nearest urgent care on your screen. Delayed treatment is the number one way insurers deny claims."
"""

MEDICAL_NEARBY_APPEND = (
    " Even if you feel okay right now — get checked today, not tomorrow. "
    "I've pulled up the nearest urgent care on your screen."
)


def _append_medical_nearby_if_needed(response: str, context: dict) -> str:
    if context.get("nearby_medical_fired") and MEDICAL_NEARBY_APPEND.strip() not in response:
        return f"{response.rstrip()}{MEDICAL_NEARBY_APPEND}"
    return response

RESPONSE_LENGTH_RULES = """
RESPONSE LENGTH — HARD LIMITS:
- Questioning phase: 2 sentences MAX
  Sentence 1: 2-4 words only ("Got it." "I hear you." "Okay.")
  Sentence 2: ONE question — use the exact wording given
...
def _build_system_prompt(
    ...
    *,
    ...
    if phase == "summarize" or (first_guiding and context.get("nearby_medical_fired")):
...
async def _run_demo_tools(stack: IncidentStack, tools_fired: list[str]) -> None:
    incident = stack.data.get("incident_type") or "car_accident"
    state = stack.data.get("state") or "Texas"
...
def _apply_final_demo_turn(stack: IncidentStack, demo: dict[str, Any]) -> dict:
    """Apply hardcoded final demo script result to stack + return agent payload."""
    apply_demo_context(stack.data, demo)

    for tool in demo.get("tools_fired") or []:
    ...
    return {
        ...
        "reasoning": demo.get("reasoning", "Final demo script"),
        "phase": phase,
        "context": stack.data,
        "demo_turn": demo.get("turn"),
...
async def _apply_final_demo_turn_async(
    stack: IncidentStack, demo: dict[str, Any]
) -> dict:
    result = _apply_final_demo_turn(stack, demo)
...
async def _questioning_turn(
    ...
    content = _build_crisp_questioning_response(transcript, next_q)
    ...
    if demo_turn:
        if stack.data.get("last_demo_turn") == demo_turn.get("turn"):
            ...
            if cached:
                ...
                return {
                    "response": cached,
                    "tool_called": demo_turn.get("tool_called"),
                    "reasoning": demo_turn.get("reasoning"),
                    "phase": demo_turn.get("phase", "questioning"),
                    "context": stack.data,
                    "demo_turn": demo_turn.get("turn"),
                    "dashboard_delay_s": demo_turn.get("dashboard_delay_s", 0),
                    "emit_nearby_medical": demo_turn.get("emit_nearby_medical", False),
                    "emit_nearby_legal": demo_turn.get("emit_nearby_legal", False),
                    "profile": profiler.checkpoints,
                }
    ...
    if stack.data.get("final_demo_active"):
        ...
        if fallback:
            demo_turn = {
                ...
                "tool_called": fallback.get("tool"),
                "phase": fallback.get("phase", "guiding"),
                "tools_fired": list(fallback.get("tools_fired") or []),
                "reasoning": fallback.get("reasoning", "Final demo script turn 6"),
                "context": dict(fallback.get("context") or {}),
                "dashboard_delay_s": fallback.get("dashboard_delay_s", 0),
                "emit_nearby_medical": bool(fallback.get("emit_nearby_medical")),
                "emit_nearby_legal": bool(fallback.get("emit_nearby_legal")),
            }
            result = await _apply_final_demo_turn_async(stack, demo_turn)
            stack.data["last_demo_response"] = result["response"]
            profiler.mark("first_token_streamed")
...
async def _run_phase_loop(
    ...
    if first_guiding and stack is not None:
        ...
        if jake_response:
            jake_response = _append_medical_nearby_if_needed(jake_response, context)
            profiler.mark("response_complete")
            return {
